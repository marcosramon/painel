...
// 1. IMPORTAÇÕES DO FIREBASE (Adicionadas)
import { initializeApp } from 'firebase/app';
import { getFirestore, collection, addDoc, getDocs } from 'firebase/firestore';

// 2. CONFIGURAÇÃO DO FIREBASE (Cole as suas credenciais aqui)
    ...
    >
      <defs>
        ...
        >
          ...
          <circle cx="110" cy="15" r="1.4" fill="#fff" />
          <circle cx="35" cy="70" r="1" fill="#fff" />
          <circle cx="85" cy="65" r="1.3" fill="#fff" />
          <circle cx="15" cy="110" r="1.1" fill="#fff" />
          <circle cx="70" cy="100" r="1" fill="#fff" />
          <circle cx="105" cy="95" r="1.2" fill="#fff" />
          <line
            x1="10"
            y1="10"
            x2="60"
...
export default function App() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (alreadySubmitted) {
      setError('Já existe uma resposta com esse nome.');
      return;
    }
    setError('');
    setSubmitting(true);
  ...
  // Restante da lógica inalterada (useMemos, cores, renderização)...
  const stateData = useMemo(() => {
    const f = {};
    entries.forEach((e) => {
      f[e.estado] = (f[e.estado] || 0) + 1;
    });
  ...
  const formacaoData = useMemo(() => {
    ...
    entries.forEach((e) => {
      const k =
        e.formacao.charAt(0).toUpperCase() + e.formacao.slice(1).toLowerCase();
      f[k] = (f[k] || 0) + 1;
    });
    return Object.entries(f)
      .sort((a, b) => b[1] - a[1])
  ...
  const profissaoData = useMemo(() => {
    ...
    entries.forEach((e) => {
      const k =
        e.profissao.charAt(0).toUpperCase() +
        e.profissao.slice(1).toLowerCase();
      f[k] = (f[k] || 0) + 1;
    });
    return Object.entries(f)
      .sort((a, b) => b[1] - a[1])
  ...
  const wordCloudData = useMemo(
    () => generateWordCloud(entries.map((e) => e.expectativas)),
    [entries]
  );
  ...
  return (
    <div
      style={{ minHeight: '100vh', background: T.gray100, fontFamily: T.font }}
    >
      <link
      ...
      >
        {[
          ...
          <button
            ...
            onClick={() => {
              setView(tab.id);
              if (tab.id === 'dashboard') refreshData();
            }}
            style={{
              flex: 1,
              padding: '13px 16px',
              ...
              cursor: 'pointer',
              fontFamily: T.font,
              color: view === tab.id ? T.blue700 : T.gray400,
              borderBottom:
                view === tab.id
                  ? `2px solid ${T.blue700}`
          ...
          <div style={{ background: T.white, padding: '28px 24px' }}>
            {submitted ? (
              ...
              <>
                ...
                >
                  Preencha os campos abaixo para se apresentar à turma. Seu nome
                  serve apenas para controle interno e não será exibido nos
                  resultados.
                </p>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <input
                    ...
                    style={inputBase}
                    onFocus={(e) => (e.target.style.borderColor = T.blue600)}
                  ...
                  {alreadySubmitted && (
                    ...
                    >
                      Já existe uma resposta com esse nome.
                    </div>
                  )}
                </div>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <textarea
                    ...
                    placeholder="Escreva livremente sobre o que espera deste curso, como pretende aplicar o que aprender, quais são seus interesses principais..."
                    rows={4}
                    style={{
                      ...inputBase,
                      resize: 'vertical',
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: '12px',
                    background:
                      submitting || alreadySubmitted ? T.gray200 : T.blue700,
              ...
              <>
                ...
                >
                  {[
                    ...
                    {
                      n: new Set(entries.map((e) => e.estado)).size,
                      label: 'estados',
                    },
                    {
                      n: new Set(entries.map((e) => e.profissao.toLowerCase()))
                        .size,
                      label: 'profissões',
                    },
                  ].map((c, i) => (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        <XAxis
                          type="number"
                          allowDecimals={false}
                          tick={{ fontSize: 11, fill: T.gray400 }}
                          axisLine={false}
                          tickLine={false}
                        />
                        <YAxis
                ...
                {cityData.length > 0 && (
                  ...
                  >
                    ...
                    <div
                      style={{ height: Math.max(160, cityData.length * 32) }}
                    >
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart
                          data={cityData}
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              ...
                              labelLine={{ stroke: T.gray200, strokeWidth: 1 }}
                              style={{ fontSize: 10, fontFamily: T.font }}
                            >
                ...
                {wordCloudData.length > 0 && (
                  ...
                  >
                    ...
                    >
                      Palavras mais frequentes nos textos de expectativa
                    </p>
